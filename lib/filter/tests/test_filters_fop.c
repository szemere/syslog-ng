...
ParameterizedTestParameters(filter_op, test_or_evaluation)
{
  static FilterParams test_data_list[] =
  {
    ...
    {.config_snippet = "    facility(3) or not facility(3)", .expected_result = FALSE },
    {.config_snippet = "not facility(3) or     facility(3)", .expected_result = FALSE },
...
{
  const gchar *msg = "<2> openvpn[2499]: PTHREAD support initialized";
  FilterExprNode *filter = _compile_standalone_filter(params->config_snippet);
  testcase(msg, filter, params->expected_result);
}


Test(filter_op, replace_existing_child)
{
  FilterExprNode *left = filter_level_new(0);
  FilterExprNode *right = filter_level_new(0);
  FilterExprNode *new = filter_level_new(0);


  FilterExprNode *filter = fop_or_new( left, right );
  FilterOp *filter_op = (FilterOp *)filter;

  cr_assert_eq(filter_op->left,  left,  "Left child not registered.");
  cr_assert_eq(filter_op->right, right, "Right child not registered.");

  filter_expr_replace_child(filter, left, new);
  cr_assert_eq(filter_op->left, new, "Filter or didn't replace the child element.");
  cr_assert_eq(filter_op->right, right, "Filter or shouldn't replace this child element.");

  filter_expr_unref(left);
  filter_expr_unref(filter); // Implicitly unrefs &right and &new
}
